...
const cds = (await asyncmap(executables, async function mapChromedriver (executable) {
      ...
      let version = match[1];
      const coercedVersion = semver.coerce(version, true);
      let expectMinChromeVersion;
      if (coercedVersion) {
        ...
        if (coercedVersion.major < NEW_CD_VERSION_FORMAT_MAJOR_VERSION) {
          ...
          // each chromedriver(>=73) support Chrome with matching major, minor, and build version.
          // see : http://chromedriver.chromium.org/downloads/version-selection
          expectMinChromeVersion = `${coercedVersion.major}.${coercedVersion.major}.${coercedVersion.patch}`
        }
      }
      return {
        executable,
        version,
        minChromeVersion: mapping[version] ? mapping[version] : expectMinChromeVersion,
      };
    }))
      .filter((cd) => !!cd)
